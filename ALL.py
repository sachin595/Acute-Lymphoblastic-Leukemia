...
def download_model():
    try:
        if not os.path.exists('komati_rf.joblib'):
            url = 'https://drive.google.com/uc?id=1Q1QO_oj06heY9MtqIleTyjpoGxq8cDrt'
            gdown.download(url, 'komati_rf.joblib', quiet=False)
        if not os.path.exists('myscale_rf.joblib'):
            url = 'https://drive.google.com/uc?id=1-z-b2fN6z8KIBiWePze9sa0Vld_W_6Wg'
...
if submit_button:
    ...
    st.write('Survival Rate:', y_pred_rounded[:,1])
