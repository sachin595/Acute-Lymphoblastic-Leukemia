...
def download_model():
    try:
        if not os.path.exists('komati_rf.joblib'):
            ...
            logging.debug('Downloaded komati_rf.joblib')
        if not os.path.exists('myscale_rf.joblib'):
            ...
            logging.debug('Downloaded myscale_rf.joblib')
    except Exception as e:
        st.write(f"Failed to download the model or scaler: {e}")
        logging.error(f"Failed to download the model or scaler: {e}")
...
try:
    ...
    logging.error(f"Scaler not loaded: {e}")
...
def predict(inputs):
    ...
    agegroup_map = {
        '1-4': 0, '5-9': 1, '10-14': 2, '15-19': 3, '20-24': 4, '25-29': 5,
        '30-34': 6, '35-39': 7, '40-44': 8, '45-49': 9, '50-54': 10, '55-59': 11,
        '60-64': 12, '65-69': 13, '70-74': 14, '75-79': 15, '80-84': 16, '85+': 17
    }
    ...
    logging.debug(f'Processed input data: {inputs}')
    ...
    logging.debug(f'Standardized input data: {input_df_scaled}')
...
if submit_button:
    ...
    logging.debug(f'User input data: {input_data}')
    ...
    y_predicted_rounded[:, 1] = np.round(y_pred[:, 1], 6)  # Round "Survival Rate" to 6 decimal places

    logging.debug(f'Rounded predictions: {y_predicted_rounded}')
